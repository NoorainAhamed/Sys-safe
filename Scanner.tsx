...
import { Shield, ShieldAlert, ShieldCheck, Search, Loader2, Activity, Globe, Lock, Cpu, Smartphone, AlertTriangle, AlertCircle } from 'lucide-react';
...
export default function Scanner() {
  ...
  return (
    <div className="max-w-7xl mx-auto p-6 md:p-8">
             ...
             <section className="p-5 rounded-xl border border-white/5 bg-[#151921] shadow-2xl">
                ...
                <ul className="space-y-3">
                  {auditResult.threats.map((threat, i) => {
                    ...
                    return (
                      <li key={threat.id || i} className={`flex gap-3 text-xs leading-relaxed transition-all duration-300 ${isCritical ? 'text-white font-bold bg-cyber-red/5 p-2 rounded border border-cyber-red/20 shadow-[0_0_15px_rgba(239,68,68,0.1)]' : 'text-slate-300 px-2'}`}>
                         <span className="flex-shrink-0 mt-0.5">
                           {threat.severity === 'CRITICAL' ? (
                             ...
                             <AlertCircle className="w-3.5 h-3.5 text-cyber-amber opacity-60" />
                           )}
                         </span>
                         <div className="flex flex-col gap-0.5">
                            <span className="flex items-center gap-2">
                              {threat.description}
                              {isCritical && (
                                <span className="text-[8px] bg-cyber-red text-white px-1 rounded-sm tracking-tighter uppercase font-black">
                                  {threat.severity}
                                </span>
                              )}
                            </span>
      ...
      {/* Playful Status Emoji Companion */}
      <motion.div 
        initial={{ opacity: 0, scale: 0.8 }}
        animate={{ opacity: 1, scale: 1 }}
        className="fixed bottom-16 right-6 md:bottom-20 md:right-10 z-[100] group cursor-pointer"
      >
        <motion.div 
          initial={{ opacity: 0, y: 10 }}
          animate={{ opacity: 1, y: 0 }}
          className="absolute right-0 bottom-full mb-4 bg-cyber-panel border border-white/10 rounded-xl p-3 text-[11px] w-40 shadow-2xl transition-all duration-300 pointer-events-none group-hover:scale-105"
        >
          <div className="absolute -bottom-2 right-6 w-4 h-4 bg-cyber-panel border-r border-b border-white/10 rotate-45" />
          <p className="text-white font-medium leading-normal relative z-10">
            {isScanning ? "Analyzing the digital ether... hang tight! 🕵️‍♂️" : 
             auditResult ? (auditResult.securityScore > 80 ? "Looks clean enough for a space mission! 😎" : "Ouch, found some dust bunnies in the code... 😬") : 
             "System idle. Bored. Want to scan? 🚀"}
          </p>
        </motion.div>
        
        <div className="bg-cyber-panel border border-white/10 w-14 h-14 rounded-2xl shadow-[0_0_20px_rgba(14,165,233,0.1)] flex items-center justify-center text-3xl hover:shadow-[0_0_30px_rgba(14,165,233,0.3)] transition-all duration-300 border-b-4 border-b-cyber-accent group-hover:rotate-12">
          {isScanning ? "🤖" : auditResult ? (auditResult.securityScore > 80 ? "🦾" : "🧨") : "👾"}
        </div>
      </motion.div>
    </div>
  );
